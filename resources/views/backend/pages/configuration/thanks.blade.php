            ...
            <div class="card mb-4">
                ...
                <form action="{{ route('configuration.thanks.save') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control {{ ($errors->any() && $errors->has('description')) ? 'is-invalid' : '' }}" placeholder="Insert Description" cols="10" rows="5">{{ old('description', isset($thanks->description) ? $thanks->description : '') }}</textarea>
                        @if ($errors->any() && $errors->has('description'))
                            <div class="invalid-feedback">{{ $errors->first('description') }}</div>
                        @endif
                    </div>
                    <div class="mb-3">
                        <img class="image-output" id="image_output" name="image_output" src="{{ asset('images/'.(isset($thanks->image) ? $thanks->image : '')) }}" alt="Image Output">
                      </div>
                    <div class="mb-3">
