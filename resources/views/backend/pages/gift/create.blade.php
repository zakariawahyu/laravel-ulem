...
@section('content')
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Gift /</span> Create
    </h4>
        ...
        <div class="col-xl">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Form Create Gift</h5>
                </div>
                <div class="card-body">
                <form action="{{ route('gift.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="bank" class="form-control {{ $errors->has('bank') ? 'is-invalid' : '' }}">
                            ...
                            @foreach ($banks as $key => $item)
                                <option value="{{ $key }}" {{ old('bank') == $key ? 'selected' : '' }}>{{ $item }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('bank'))
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="account_name" class="form-control {{ $errors->has('account_name') ? 'is-invalid' : '' }}" placeholder="Insert Account Name" value="{{ old('account_name') }}" />
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="account_number" class="form-control {{ $errors->has('account_number') ? 'is-invalid' : '' }}" placeholder="Insert Account NUmber" value="{{ old('account_number') }}" />
